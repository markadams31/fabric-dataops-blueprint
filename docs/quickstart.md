...
| Git integration → **Users can sync workspace items with GitHub repositories** | Enabled, scoped to `grp-fabric-automation` **and `grp-fabric-workspace-creators`** | Fabric's GitHub provider is off by default. Both groups are needed: automation for the pipeline, and workspace creators because that is who connects a branched workspace to a branch. Scoped to just the first, a developer's connection attempt fails with `FeatureNotAvailable` and no hint as to which setting is missing. |
