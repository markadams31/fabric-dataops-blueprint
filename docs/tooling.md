...
| **Developers create their own branched workspaces** | Provision them through a dispatchable workflow, or declare them in Terraform | Self-service with no platform team in the loop, which suits a single-maintainer demonstration | It takes two tenant-level grants — workspace creation and capacity contributor — that cannot be narrowed to this use, and many organisations will not give them out. [The alternatives](branched-workspaces.md#before-you-start) keep the experience and move the privilege into the pipeline |
