...
|---|---|---|---|
| `fabric_warehouse` | **GA**, SPN | The `.platform`-only folder fabric-cicd publishes to conjure an empty warehouse | **Declined 2026-09-04.** A warehouse is data plane, so an infrastructure tool is the wrong owner: Terraform's remedy for a changed immutable attribute is to replace the resource, and for the only copy of the marts that remedy is data loss. It would also contradict D10, which keeps data-bearing types out of orphan control for the same reason. The `MissingItemDefinitionFiles` blocker it would have removed stays, documented, as the cost |
